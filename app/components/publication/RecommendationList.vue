<template>
  <div class="flex flex-col gap-6">
    ...
    <div class="flex flex-col gap-4">
      <div v-for="(rec, i) in recommendations" :key="i" class="flex gap-2">
        ...
        <div class="flex flex-col gap-">
          <div class="flex items-center justify-between">
            <p class="text-sm font-bold text-on-surface">{{ rec.titulo }}</p>
            <span class="text-xs rounded px-1.5 py-0.5" :class="priorityConfig[rec.prioridad]?.color">
              {{ priorityConfig[rec.prioridad]?.label }}
            </span>
          </div>
          <p class="text-sm text-on-surface">{{ rec.descripcion }}</p>
        </div>
      </div>
    </div>
  </div>

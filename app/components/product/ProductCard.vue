<template>
  <NuxtLink :to="`${ROUTE_NAMES.PRODUCTOS}/${product.id}`"
    ...
    <div class="w-40 h-40 shrink-0 bg-surface-lowest rounded-xl overflow-hidden relative">
    ...
    <div class="flex-1 flex flex-col gap-3 min-w-0">
      <div class="flex justify-between items-start">
        ...
        <SharedTrendBadge :value="product.trend" size="md" />
      </div>
      ...
      <div class="grid grid-cols-3 gap-2 border-t border-outline-variant/50 pt-3">
        <div>
          <p class="text-sm text-on-surface-variant">Publicaciones</p>
          <p class="font-semibold text-on-surface">{{ product.publicacionesActivas }} activas</p>
        </div>
        <div>
          <p class="text-sm text-on-surface-variant">Stock</p>
          <p class="font-semibold"
            :class="isOutOfStock(product) ? 'text-red-600 dark:text-red-400' : 'text-on-surface'">
            {{ product.stockTotal }} un.
          </p>
        </div>
        <div>
          <p class="text-sm text-on-surface-variant">Revenue 30d</p>
          <p class="font-semibold text-primary-600 dark:text-primary-400">{{ formatRevenue(product.revenue30d) }}</p>
        </div>
      </div>
    </div>
  </NuxtLink>
